...
function level_of_node(j::Integer, n::Integer)
    @assert j ≥ 0 && n ≥ 2
    i = 0
    while true
...
"""
    populate!(model::MyGeneralDiscreteRecombiningTreeModel, price::Float64, Δ::Array{Float64,1})
...
### Arguments
- `model::MyGeneralDiscreteRecombiningTreeModel`: The tree model to populate.
- `quantity::Float64`: The initial quantity to set at the root of the tree.
- `Δ::Array{Float64,1}`: The array of quantity change factors for each level of the tree.

"""
function populate!(model::MyGeneralDiscreteRecombiningTreeModel, 
    quantity::Float64, Δ::Array{Float64,1})::MyGeneralDiscreteRecombiningTreeModel
    ...
    P[0] = (quantity = quantity, path = k) # set root quantity
    ...
    for i ∈ 0:(Nₕ - 1)
        ...
        for j ∈ 1:number_of_moves
            tmp *= Δ[j]^(k[j]) # accumulate the quantity factor for each move
        end
        P[i] = (quantity = quantity * tmp, path = k);
    end
