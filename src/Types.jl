"""
    mutable struct  MyGeneralRecombiningTreeModel <: AbstractPriceTreeModel

The `MyGeneralRecombiningTreeModel` type is a model of a commodity price tree that uses an dictionary to store the price data.
...
mutable struct MyGeneralRecombiningTreeModel <: AbstractPriceTreeModel
    ...
    # constructor 
    MyGeneralRecombiningTreeModel() = new()
end
